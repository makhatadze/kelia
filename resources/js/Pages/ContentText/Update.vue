    ...
    data() {
        return {
            form: this.$inertia.form({
                ...
                tag: this.item?.tag ? parseInt(this.item.tag) : 10,
                section: this.item?.section ? parseInt(this.item.section) : 1000,
                text_type: this.item?.text_type ? parseInt(this.item.text_type) : 1,
            }),
        }
    },
    methods: {
        submitForm() {
            this.form.post(route('content-text.update',this.item.id), {
                preserveScroll: true,
                forceFormData: true,
                onSuccess: () => (this.form.reset('title','tag','section','text_type')),
            });
        },
    },
})
