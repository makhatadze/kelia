...
class Helper {
    public static function finder(Collection $texts, int $section, int $textType) {
        $item = $texts->first(function (ContentText $item) use ($section, $textType) {
            return $item->section == $section && $item->text_type == $textType;
        });
    ...
    public static function finderImage(Collection $texts, int $location): string
    {
        $item = $texts->first(function (ContentImage $item) use ($location) {
            return $item->tag == $location;
        });
    ...
    public static function finderSimple(Collection $texts, int $section) {
        $item = $texts->first(function (ContentText $item) use ($section) {
            return $item->section == $section;
        });

        if ($item && $item->title_geo) {
            return $item->title_geo;
        }
    }

    private static function returnIfExists($key, array $array) {
...
//            'footerType' => $allOptions['footerType'][$data['footerType']],
            ...
            'horizontalMenuClass' => self::returnIfExists($data['horizontalMenuType'], $allOptions['horizontalMenuClass']),
            'navbarClass' => self::returnIfExists($data['verticalMenuNavbarType'], $allOptions['navbarClass']),
            'horizontalMenuType' => self::returnIfExists($data['horizontalMenuType'], $allOptions['horizontalMenuType']),
            'footerType' => self::returnIfExists($data['footerType'], $allOptions['footerType']),
            'verticalMenuNavbarType' => self::returnIfExists($data['verticalMenuNavbarType'], $allOptions['verticalMenuNavbarType']),
